  ...
  void Run(In_TextDocumentComplete *request) override {
    ...
    if (result.ok) {
      ...
      CompletionManager::OnComplete callback =
          [this, existing_completion, has_open_paren, id = request->id,
           params = request->params](CodeCompleteConsumer *OptConsumer) {
            ...
            if (!Consumer->from_cache) {
              std::string path = params.textDocument.uri.GetPath();
              completion_cache->WithLock([&]() {
                completion_cache->cached_path_ = path;
                completion_cache->cached_completion_position_ = params.position;
                completion_cache->cached_results_ = Consumer->ls_items;
              });
            }
          };
...
#endif
      if (completion_cache->IsCacheValid(params)) {
        CompletionConsumer Consumer(CCOpts, true);
        completion_cache->WithLock([&]() {
          Consumer.ls_items = completion_cache->cached_results_;
        });
        callback(&Consumer);
      } else {
